	...
	async start() {
		...
		this.server?.on('upgrade', async (request, socket, head) => {
			try {
				...
				if (await this.verifyToken(token!)) {
					...
					// Upgrade connection and fire the connection handler
		...
		this.app.get('/logs/', async (request, response) => {
			...
			try {
				...
				const logs = $({all: true})`docker logs ${containerName}`
				await pipeline(logs.all!, createGzip(), response)
			} catch (error) {
				this.logger.error(`Error streaming logs`, error)

				if (!response.headersSent) {
					response.status(500).send('Failed to download logs')
				}
			}
		})
